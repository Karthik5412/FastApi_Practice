...
# dimensions
class dimension_of_product_update(BaseModel) :
    weight : Optional[float]= Field(le= 25, ge= 5, default= 5)
    height : Optional[float]= Field(le= 2, ge= 0.2, default= 0.5)
    depth : Optional[float]= Field(le= 50, ge= 5, default= 10)

# Seller Model
class seller_model_update(BaseModel):
    seller_name : Optional[str]= Field(min_length=7,max_length= 25, default='John Doe')
    seller_rating : Optional[float]=  Field(lt= 5, gt= 0, default= None)
    is_verified : Optional[bool]=  Field(default= None, strict= True)
...
class product_update(BaseModel):
    ...
    # "price"
    price : Optional[float] = Field(ge=30, title='Price', examples=['1249.99'])
    
    
    # "is_in_stock"
    is_in_stock : Optional [bool] = Field(title= 'In stock', strict= True)
    
    # "dimensions"
    dimensions : Optional[dimension_of_product_update]
    
    # "tags"
    tags : Optional[List[str]] = Field(title= 'Tags', default= None, max_length= 10, description='Category of product')
    
    # "manufacturer_email"
    manufacturer_email : Optional[EmailStr] = Field(examples=['johdoe@gmail.com'])
    
    # "rating": 
    rating : Optional[float] = Field(default=2.25, lt=5, gt=0, description='Rate about the product')
    
    # "warehouse_location"
    warehouse_location : Optional[str] = Field(max_length= 5, min_length=3, examples=["WA-01"])
    
    
    # "shipping_code"
    shipping_code : Optional[str]=Field(max_length= 50, min_length=15, examples=["SHP-WA-01-c552d4e8-128f-4609-ae06-42310fd7b35c"])
    
    
    #"quantity_available"
    quantity_available : Optional[int]=Field(ge=0, description='Quantity of product')
    
    
    #"discount"
    discount : Optional[float]= Field(default=5, le=50, ge=0, description='Available Discount')
    
    
    
    # "seller_data"
    seller_data : Optional[seller_model_update]
